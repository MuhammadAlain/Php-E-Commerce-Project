                ...
                <tbody>
                    ...
                    if (isset($con)) {
                        ...
                        if (!$get_product_result) {
                            echo "<tr><td colspan='8' class='text-danger'>Error fetching products: " . mysqli_error($con) . "</td></tr>";
                        } else {
                            $id_number = 1; // Used for sequential display ID

                            while ($row_fetch_products = mysqli_fetch_array($get_product_result)) {
                                $product_id = $row_fetch_products['product_id'];
                                $product_title = htmlspecialchars($row_fetch_products['product_title']); 
                                $product_image_one = htmlspecialchars($row_fetch_products['product_image_one']); 
                                $product_price = htmlspecialchars($row_fetch_products['product_price']);
                                $product_status = htmlspecialchars($row_fetch_products['status']); 

                                // Get total quantity sold for the current product
                                // Using prepared statements for security (though this is a SELECT query, good practice for variables)
                                $get_count_sold_query = "SELECT SUM(quantity) AS total_sold FROM `orders_pending` WHERE product_id = ?";
                                $stmt = mysqli_prepare($con, $get_count_sold_query);
                                mysqli_stmt_bind_param($stmt, "i", $product_id);
                                mysqli_stmt_execute($stmt);
                                $get_count_sold_result = mysqli_stmt_get_result($stmt);
                                $fetch_sold_data = mysqli_fetch_assoc($get_count_sold_result);
                                $quantity_sold_of_each_product = $fetch_sold_data['total_sold'] ?? 0; // Use null coalescing for safety
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href='index.php?edit_product={$product_id}' class='text-decoration-none'>
                                    ...
                                    <td>
                                        <a href='#' data-bs-toggle='modal' data-bs-target='#deleteModal_{$product_id}' class='text-decoration-none text-danger'>
                                                ...
                                                <div class='modal-content'>
                                                    <div class='modal-body text-center'>
                                                        <div class='d-flex flex-column gap-3 align-items-center'>
                                                            ...
                                                            <h2>Are you sure?</h2>
                                                            <p>
                                                                Do you really want to delete this product: <strong>{$product_title}</strong>? This process cannot be undone.
                                                            </p>
                                                            <div class='btns d-flex gap-3'>
                                                                <button type='button' class='btn px-5 btn-secondary' data-bs-dismiss='modal'>Cancel</button>
                                                                <a class='btn px-5 btn-danger' href='index.php?delete_product={$product_id}'>Delete</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        echo "<tr><td colspan='8' class='text-danger'>Database connection not established.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
