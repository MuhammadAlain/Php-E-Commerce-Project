                ...
                <tbody>
                    ...
                    if (isset($con)) {
                        ...
                        if (!$get_category_result) {
                            ...
                            echo "<tr><td colspan='4' class='text-danger'>Error fetching categories: " . mysqli_error($con) . "</td></tr>";
                        } else {
                            $id_number = 1; // Initialize a counter for display purposes

                            // Loop through each fetched category row
                            while ($row_fetch_categories = mysqli_fetch_array($get_category_result)) {
                                $category_id = $row_fetch_categories['category_id'];
                                // Sanitize category title for safe display in HTML to prevent XSS attacks
                                $category_title = htmlspecialchars($row_fetch_categories['category_title']);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href='index.php?edit_category={$category_id}' class='text-decoration-none'>
                                    ...
                                    <td>
                                        <a href='#' data-bs-toggle='modal' data-bs-target='#deleteModal_{$category_id}' class='text-decoration-none text-danger'>
                                                ...
                                                <div class='modal-content'>
                                                    <div class='modal-body text-center'>
                                                        <div class='d-flex flex-column gap-3 align-items-center'>
                                                            ...
                                                            <h2>Are you sure?</h2>
                                                            <p>
                                                                Do you really want to delete category: <strong>{$category_title}</strong>? This process cannot be undone.
                                                            </p>
                                                            <div class='btns d-flex gap-3'>
                                                                <button type='button' class='btn px-5 btn-secondary' data-bs-dismiss='modal'>Cancel</button>
                                                                <a class='btn px-5 btn-danger' href='index.php?delete_category={$category_id}'>Delete</a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        // Display an error if the database connection is not found
                        echo "<tr><td colspan='4' class='text-danger'>Database connection not established. Please check your `connect.php` file.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
